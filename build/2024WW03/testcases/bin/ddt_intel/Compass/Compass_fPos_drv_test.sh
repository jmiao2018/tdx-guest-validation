...
#
# File:
#	Compass_fPos_drv_test.sh
#
# Description:
#	This will run positive functional tests against Compass sensor driver.
#
# Setup:
...
#	LOG: test logs are stored at Compass_$TSID_$PLATFORM_$(date +%Y%m%d%H%M).log
#
#-----------------------------------------------------------------------
debug=false
...
{
cat <<__EOF >&2
Compass Driver Test Case List
   Test Case ID       Test Description
       1              Load/unload Compass driver module
       2              Bind/unbind I2C bus driver for Compass
       3              Enumerate Compass device in /sys/bus/iio/devices
       4              Enumerate Compass device in /sys/bus/acpi/devices
__EOF
exit 0
}
