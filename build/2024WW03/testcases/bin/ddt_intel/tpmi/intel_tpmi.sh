...
# Description:  Test script for Intel TPMI feature, which is supported beginning
# from Granite Rapids platform
# TPMI:Topology Aware Register and PM Capsule Interface

# Authors:      wendy.wang@intel.com
# History:      Sep 10 2021 - Created - Wendy Wang
...
INTEL_UFS_PATH="/sys/devices/system/cpu"
...
get_vsec_oobmsm_device() {
  ...
  # On DMR, the VSEC OOBMSM PCI Device ID is changed to 09a1
  if [[ -z $dev_lines ]]; then
    dev_lines=$(lspci -d 8086:09a1 | wc -l 2>&1)
  fi
  [[ -n "$dev_lines" ]] || block_test "Did not detect tpmi pci device."
}
...
tpmi_debugfs_interface() {
  ...
  if [[ -z "$tpmi_pci_device" ]]; then
    tpmi_pci_device=$(lspci -d 8086:09a1 2>&1)
  fi
  [[ -n "$tpmi_pci_device" ]] || block_test "Did not detect tpmi pci device."
  ...
  if [[ -n "$tpmi_debugfs_instance" ]] && [[ -n "$tpmi_pci_device" ]] &&
    ...
    die "intel_vsec_tpmi debugfs instance number is not correct!"
  fi
...
intel_ufs_sysfs() {
  test_print_trc "Check Intel_uncore_frequency interface:"

  [[ -d "$INTEL_UFS_PATH"/intel_uncore_frequency ]] ||
    die "Intel_uncore_frequency SYSFS does not exist!"

  lines=$(grep . "$INTEL_UFS_PATH"/intel_uncore_frequency/*/* 2>&1 |
...
dump_pfs_lock_disable_status() {
  ...
  for ((i = 1; i <= dev_lines; i++)); do
    ...
    disabled_status=$(cat "$TPMI_DEBUGFS_PATH"/tpmi-0000:"$dev_name"/pfs_dump |
