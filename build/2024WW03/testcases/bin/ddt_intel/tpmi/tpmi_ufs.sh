...
# Description:  Test script for TPMI UFS feature,
# which is supported beginning
...
# History:      May 20 2022 - Created - Wendy Wang
...
[[ -e "$UFS_SYSFS_PATH" ]] || block_test "/sys/devices/system/cpu/intel_uncore_frequency sysfs is not available."
...
ufs_init_min_max_value() {
  ...
  for ((i = 1; i <= per_die_num; i++)); do
    ...
    if [ "$init_min" -ne 0 ] && [ "$init_max" -ne 0 ]; then
      ...
      die "$per_die shows UFS initial uncore freq value not correct."
      do_cmd "grep . $UFS_SYSFS_PATH/$per_die/*"
    fi
  done
}
...
min_max_half() {
  ...
  # Test Uncore freq is based on per die, this function is to set
  # Min_freq_khz and max_freq_khz to the half value of initial_max_freq_khz
  per_die_num=$(ls "$UFS_SYSFS_PATH" | grep -c uncore)
  for ((i = 1; i <= per_die_num; i++)); do
    per_die=$(ls "$UFS_SYSFS_PATH" | grep uncore | sed -n "$i,1p")

    # Save the min and max freq value
    default_min=$(cat $UFS_SYSFS_PATH/"$per_die"/min_freq_khz)
    default_max=$(cat $UFS_SYSFS_PATH/"$per_die"/max_freq_khz)
    init_max=$(cat $UFS_SYSFS_PATH/"$per_die"/initial_max_freq_khz)
    [[ -n "$init_max" ]] || block_test "initial_max_freq_khz is not available"
    # Set test freq to the half of the init max freq value
    test_freq=$(echo "$init_max"/2 | bc)
...
equals to the half of initial_max_freq_khz."
    do_cmd "grep . $UFS_SYSFS_PATH/$per_die/*"
    ...
    test_print_trc "Recover $per_die min_freq_khz and max_freq_khz to the default value:"
    do_cmd "echo $default_min > $UFS_SYSFS_PATH/$per_die/min_freq_khz"
    do_cmd "echo $default_max > $UFS_SYSFS_PATH/$per_die/max_freq_khz"
    do_cmd "grep . $UFS_SYSFS_PATH/$per_die/*"

    # Uncore Current freq should be less than the max test freq
    if [ "$current_freq" -eq "$test_freq" ]; then
      test_print_trc "$per_die shows Uncore current freq $current_freq equals to \
max test freq $test_freq"
    else
      die "$per_die shows Uncore current freq $current_freq is beyond \
max test freq $test_freq"
    fi
...
min_max_dynamic() {
  ...
  for ((i = 1; i <= per_die_num; i++)); do
    ...
    if [[ $default_current_freq -eq $default_max ]]; then
      test_freq="$default_max"
    else
...
equals to current_freq_khz + 100000 khz"
    ...
    # Get uncore current_freq_khz after mem stress running 30 seconds
    current_freq=$(cat $UFS_SYSFS_PATH/"$per_die"/current_freq_khz)
    test_print_trc "$per_die current freq is: $current_freq"
    ...
    if [ "$current_freq" -eq "$test_freq" ]; then
      test_print_trc "$per_die shows Uncore current freq $current_freq equals to \
min and max test freq $test_freq"
    else
      die "$per_die shows Uncore current freq $current_freq is beyond \
min and max test freq $test_freq"
    fi
...
# Function to put SUT into S4 and resume
s4_entry_resume() {
  local duration=120
  local disk_entry=""
  local disk_resume=""
...
tpmi_ufs_test() {
  ...
  load_unload_ufs_tpmi)
    load_unload_module intel_uncore_frequency_tpmi
    ;;
  ...
  check_per_pkg_min_max_change)
    pkg_max_min_freq_change
    ;;
  check_ufs_current_dynamic_after_s4)
    s4_entry_resume
    min_max_dynamic
    ;;
  esac
