...
#
# File:
#	Gyroscope_fPos_drv_test.sh
#
# Description:
#	This will run positive functional tests against Gyroscope sensor driver.
#
# Setup:
...
#		3		Test Gyroscope device is enumerated in /sys/bus/iio/devices
#		4		Test Gyroscope device is enumerated in /sys/bus/acpi/devices
...
#	LOG: test logs are stored at Gyroscope_$TSID_$PLATFORM_$(date +%Y%m%d%H%M).log
#
#-----------------------------------------------------------------------
debug=false
...
{
cat <<__EOF >&2
Gyroscope Driver Test Case List
   Test Case ID       Test Description
       1              Load/unload Gyroscope driver module
       2              Bind/unbind I2C bus driver for Gyroscope
       3              Enumerate Gyroscope device in /sys/bus/iio/devices
       4              Enumerate Gyroscope device in /sys/bus/acpi/devices
__EOF
exit 0
}
