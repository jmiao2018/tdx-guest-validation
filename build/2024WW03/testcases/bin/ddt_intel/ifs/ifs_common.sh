...
# @Desc  Test script to verify IFS(In Field Scan, origin named SAF:Scan And
#        Field) functionality
  ...
  for line in $lines; do
    case $case in
      ...
      "$CASE_TWICE")
        ...
        ;;
      "$IMG_VERSION")
        # Could check image version for atom CPU by rdmsr 0x2c8
        #is_atom
  ...
  case $case in
    "$IFS_OFFLINE")
      if [[ "$cpus" == "all" ]]; then
        ...
        # Met https://hsdes.intel.com/appstore/article/#/15014801758 problem
        # Interrupt occurred prior to SCAN coordination reason could skip.
        #for cpu in $CPU_LIST; do
...
ifs_array_off_cpu_scan() {
  ...
  elif [[ "$ifs_func" == "$SBFT" ]]; then
    # TODO, will add the key word dmesg check for IFS SBFT next step.
    test_print_trc "IFS $SBFT test, no key word dmesg check."
  else
...
# ifs SBFT scan test, offline CPU and SBFT scan should pass
# Input $1: cpu num will load 100% and scan
# Output: 0 otherwise failure or die
ifs_sbft_cpu_fullload_scan() {
  local cpus=$1
  local cpu=""
...
# SBFT(Structural Based Fault Test) scan with interrupt in target CPU test
# Input $1: cpu num will load 100% and scan
# Output: 0 otherwise failure or die
sbft_int_test() {
  local cpu=$1
