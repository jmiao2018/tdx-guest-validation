...
test_ifs() {
  ...
  case $NAME in
    ...
    "ifs_array_cpuran_fullload")
      ...
      ;;
    "ifs_sbft_cpuran_fullload")
      enable_ifs_trace

      # Load the ifs sbft batch
      if [[ -e "${IFS_PATH}/${BATCH}" ]]; then
        test_print_trc "echo $BATCH_NUM > ${IFS_PATH}/${BATCH}"
        echo "$BATCH_NUM" > "$IFS_PATH"/"$BATCH"
        check_file_content "${IFS_PATH}/${BATCH}" "$BATCH_NUM" "$CONTAIN"
        check_file_content "${IFS_PATH}/${VERSION}" "none" "$NE"
      else
        block_test "No ${IFS_PATH}/${BATCH} file, is it 5.15 or old ifs kernel?"
      fi

      for((i=1;i<=TIMES;i++)); do
        list_cpus "$PROCESSOR"
        test_print_trc "  -> $i round IFS SBFT cpu:$CPU_LIST load 100% scan test:"
        for cpu in $CPU_LIST; do
          ifs_sbft_cpu_fullload_scan "$cpu"
        done
      done
      ;;
    "msr_array")
    ...
    "sbft_interrupt")
      ...
      for((i=1;i<=TIMES;i++)); do
        list_cpus "$PROCESSOR"
        test_print_trc "  -> $i round IFS SBFT & interrupt test in cpu:$CPU_LIST:"
        for cpu in $CPU_LIST; do
          sbft_int_test "$cpu"
        done
      done
    ...
    "ifs_sbft_offran")
      ...
      for((i=1;i<=TIMES;i++)); do
        list_cpus "$PROCESSOR"
        test_print_trc "  -> $i round IFS SBFT offline cpu:$CPU_LIST scan negative test:"
        ifs_array_off_cpu_scan "$CPU_LIST" "$SBFT"
      done
      ;;
    *)
