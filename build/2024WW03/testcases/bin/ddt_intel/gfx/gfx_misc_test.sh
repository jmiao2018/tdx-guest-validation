...
#test cases list of miscellaneous func test
GFX_MISC_CASES="prime_self_import ddi_compute_wrpll"
...
if [ -z $TEST_CASE ];then
	...
	GFX_MISC_CASES=$(echo $GFX_MISC_CASES | tr ' ' '\n')
	test_print_trc "The supported tests are:$GFX_MISC_CASES"
	exit 1
fi

#check if $TEST_CASE is supported in test cases list
bin_name=$(echo $GFX_MISC_CASES | tr ' ' '\n' | awk '{if(match($0,/^'$TEST_CASE'$/)) print substr($0,RSTART,RLENGTH)}')
if [ -z $bin_name ];then
...
test_print_start "GFX miscellaneous tests: $TEST_CASE is starting"
if [ -z $SUB_TEST ];then
	gfx_test.sh -t misc -b $TEST_CASE
else
	gfx_test.sh -t misc -b $TEST_CASE -s $SUB_TEST
fi
