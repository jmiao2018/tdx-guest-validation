...
# Description:  Test script for Intel SST feature, which is supported on server
# Beginning from Granite Rapids platform, only isst_tpmi driver is supported
...
# History:      Nov 25 2021 - Created - Wendy Wang
#               Apr 08 2022 - Added more CP cases - Wendy Wang
#               Jun 07 2022 - Added more GNR supported TF/BF cases - Wendy Wang

source "common.sh"
source "dmesg_functions.sh"
source "powermgr_common.sh"

ISST_TPMI_DRIVER_PATH="/sys/module/isst_tpmi/drivers/auxiliary"
...
isst_load_unload_module() {
  ISST_MODULE=$1
  is_kmodule_builtin "$ISST_MODULE" && skip_test
  dmesg -C

  load_unload_module.sh -c -d "$ISST_MODULE" &&
    do_cmd "load_unload_module.sh -u -d $ISST_MODULE"
  do_cmd "load_unload_module.sh -l -d $ISST_MODULE"
  do_cmd "load_unload_module.sh -u -d $ISST_MODULE"
  do_cmd "load_unload_module.sh -l -d $ISST_MODULE"
}

isst_tpmi_driver_interface() {
  test_print_trc "Check ISST_TPMI driver interface:"

  [[ -d "$ISST_TPMI_DRIVER_PATH":isst_tpmi ]] ||
    die "ISST_TPMI driver SYSFS does not exist!"

  lines=$(ls "$ISST_TPMI_DRIVER_PATH":isst_tpmi 2>&1)
...
isst_base_freq_pp_level_change() {
  ...
  iodie_number=$(grep -c "cpu--1" pp_info.out)
  if [[ -n "$iodie_number" ]]; then
    package_number=$(("$package_number" - "$iodie_number"))
  fi
  test_print_trc "Package number: $package_number"
  for ((i = 1; i <= "$package_number"; i++)); do
    enabled_cpu=$(cat pp_info.out | grep -A 10 "perf-profile-level-$level_id" |
    ...
    base_freq_af_khz=$(cat "$CPU_SYSFS_PATH"/cpu"$enabled_cpu"/cpufreq/base_frequency)
    ...
    base_freq_tool=$(cat pp_info.out | grep -A 10 "perf-profile-level-$level_id" |
...
#Funtion to check isst core power feature enable and disable
...
#Funtion to set isst core power configuration for clos id and min freq
isst_cp_config_clos_min_set_get() {
  local clos_id=$1
  local min_khz=""
  min_khz=$(cat /sys/devices/system/cpu/cpu0/cpufreq/base_frequency)
...
#Funtion to set isst core power configuration for clos id and max freq
...
#Funtion to set isst core power configuration for clos id and clos-proportional-priority
...
isst_bf_baseline_test() {
  ...
  base_freq_sysfs=$(cat "$CPU_SYSFS_PATH"/cpu0/cpufreq/base_frequency)
  ...
  do_cmd "$PSTATE_TOOL/turbostat -q --show $COLUMNS -i 10 -o tc.out sleep 10"
  test_print_trc "Turbostat output when 100% workload stress running is:"
  do_cmd "cat tc.out"

  #first_cpu_freq_temp=$(echo "$cpu_stat" | tail -n $cpu_num_all | head -n 1 | awk '{print $5}')
  first_cpu_freq_temp=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
  ...
  #last_cpu_freq_temp=$(echo "$cpu_stat" | tail -n 1 | head -n 1 | awk '{print $5}')
  last_cpu_freq_temp=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
...
isst_bf_freq_test() {
  ...
  for ((i = 1; i <= hp_bf_cpus_num; i++)); do
    ...
    do_cmd "$PSTATE_TOOL/turbostat -q -c 0,$hp_bf_cpus_2nd,$hp_bf_cpus_3rd --show $COLUMNS -i 10 -o tc.out sleep 10"
    test_print_trc "Turbostat output when 100% workload stress running is:"
    do_cmd "cat tc.out"

    #Assume CPU0 is the low priority CPU
    actual_lp_cpu0_freq=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
      grep "Bzy_MHz" | awk -F " " '{print $3}')
    ...
    actual_2nd_cpu_freq=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
    ...
    actual_3rd_cpu_freq=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
...
isst_tf_freq_test() {
  local level_id=$1
  local model
  model=$(grep model /proc/cpuinfo | awk -F ": " '{print $2}' | sed -n '1,1p')
  ...
  do_cmd "$PSTATE_TOOL/turbostat -q -c 0-14 --show $COLUMNS -i 10 -o tc.out sleep 10"
  ...
  #base_turbo_freq_cpu8=$(echo "$cpu_stat" | tail -n 7 | head -n 1 | awk '{print $5}')
  base_turbo_freq_cpu8=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
    grep "Bzy_MHz" | awk -F " " '{print $11}')
  test_print_trc "The CPU8 base turbo freq is: $base_turbo_freq_cpu8 MHz"
  #base_turbo_freq_cpu9=$(echo "$cpu_stat" | tail -n 6 | head -n 1 | awk '{print $5}')
  base_turbo_freq_cpu9=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
  ...
  if [[ "$model" == 173 ]]; then
    opportunistic_hp_tf=$(grep "high-priority-max-level-0-frequency(MHz)" tf.out | awk -F ":" '{print $2}' | sed -n "1, 1p")
  else
    opportunistic_hp_tf=$(grep "high-priority-max-frequency(MHz)" tf.out | awk -F ":" '{print $2}' | sed -n "1, 1p")
  fi
  test_print_trc "ISST bucket 0 marked high priority turbo freq is: $opportunistic_hp_tf MHz"
  ...
  hp_turbo_freq_cpu8=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
    grep "Bzy_MHz" | awk -F " " '{print $11}')
  test_print_trc "The high priority CPU8 turbo freq is: $hp_turbo_freq_cpu8 MHz"
  hp_turbo_freq_cpu9=$(awk '{for(k=0;++k<=NF;)a[k]=a[k]?a[k] FS $k:$k} END{for(k=0;k++<NF;)print a[k]}' tc.out |
    grep "Bzy_MHz" | awk -F " " '{print $12}')
  ...
  #Kill stress threads
  [ -n "$stress_pid" ] && do_cmd "do_kill_pid $stress_pid"
  #[ -n "$perf_pid" ] && do_cmd "do_kill_pid $perf_pid"
...
#Funtion to put SUT into S4 and resume
isst_s4_entry_resume() {
  local duration=120
  local disk_entry=""
  local disk_resume=""

  do_cmd "rtcwake -m disk -s $duration 2>&1"
  do_cmd "sleep 60"

  disk_entry=$(dmesg | grep "hibernation entry")
  disk_resume=$(dmesg | grep "hibernation exit")

  if [ -n "$disk_entry" ] && [ -n "$disk_resume" ]; then
    test_print_trc "Hibernation entry and exit successfully"
  else
    die "Hibernation failed check from dmesg log."
  fi
}
...
isst_test() {
  ...
  isst_tpmi_load_unload)
    isst_load_unload_module isst_tpmi
    ;;
  isst_if_mmio_load_unload)
    isst_load_unload_module isst_if_mmio
    ;;
  isst_if_mbox_load_unload)
    isst_load_unload_module isst_if_mbox_pci
    ;;
  isst_tpmi_driver_sysfs)
    isst_tpmi_driver_interface
    ;;
  ...
  isst_hp_tf_freq_pp_level4_ordered_type_test)
    ...
    isst_tf_freq_test 4
    ;;
  isst_bf_baseline_freq_test_after_s4_resume)
    isst_s4_entry_resume
    isst_bf_baseline_test
    ;;
  isst_hp_bf_freq_test_after_s4_resume)
    isst_s4_entry_resume
    isst_bf_freq_test 0
    ;;
  isst_hp_tf_freq_test_after_s4_resume)
    isst_s4_entry_resume
    isst_tf_freq_test 0
    ;;
  esac
