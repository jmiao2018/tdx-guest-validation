...
# @desc     Check if OTG notification are present on dmesg after resume
...
check_otg_notification() {
    local power_state=$1
    local last_dmesg_time=0
    local expect=".*$PMIC_PWSRC_DRIVER $PMIC_PWSRC_DRIVER: \[OTG notification\]evt:4.*"
    shift 1

    test_print_trc "checking OTG notification after resume: $expect"
    ...
    sleep 10 # wait a little more for OTG notifications

    # Check if OTG notification was received
    if $(dmesg | sed "1,/$last_dmesg_time/d" | grep -Eqo "$expect"); then
        test_print_trc "OTG notification received"
        return 0
    else
        test_print_trc "Error, OTG notification not received"
        return 1
    fi
}
...
for i in $(seq 1 $ITERATIONS); do
    check_otg_notification ${PSTATE_ARR[$(( $RANDOM % ${#PSTATE_ARR[@]} ))]} || \
    die "Error on pmic otg notifications"
done
