...
usage()
{
	...
	-R REC_DEVICE           Device Name like plughw:0,0.
	-P PLAY_DEVICE           Device Name like plughw:0,0.
...
: ${TYPE:='loopback'}
: ${FILE:='test.snd'}
: ${REC_DEVICE:=$(get_audio_devnodes.sh -d $dev_type -t record | grep 'hw:[0-9]' || echo 'plughw:0,0')}
: ${PLAY_DEVICE:=$(get_audio_devnodes.sh -d $dev_type -t play | grep 'hw:[0-9]' || echo 'plughw:0,0')}
: ${DEVICE:=$PLAY_DEVICE}
...
do_cmd enable_all_playback "$CARD"
do_cmd enable_mic_alsa "$CARD"
...
case "$TYPE" in
	capture)
		do_cmd arecord -D $DEVICE -t "raw" -f "$SAMPLEFORMAT" $FILE -d "$DURATION" -r "$SAMPLERATE" -c "$CHANNEL" "$ACCESSTYPEARG" "$OPMODEARG" -N --buffer-size=$BUFFERSIZE --period-size $PERIODSIZE
		;;
	playback)
		wget $URL -c $FILE --timeout=10 -t 1
		...
		do_cmd aplay -D $DEVICE -f "$SAMPLEFORMAT" $FILE -d "$DURATION" -r "$SAMPLERATE" -f "$SAMPLEFORMAT" -c "$CHANNEL" "$ACCESSTYPEARG" "$OPMODEARG"  -N --buffer-size=$BUFFERSIZE --period-size $PERIODSIZE
		;;
	loopback)
		do_cmd arecord -D $DEVICE -t "raw" -f "$SAMPLEFORMAT" -d "$DURATION" -r "$SAMPLERATE" -c "$CHANNEL" "$ACCESSTYPEARG" "$OPMODEARG"  -N --buffer-size=$BUFFERSIZE --period-size $PERIODSIZE "|" aplay -D $DEVICE -f "$SAMPLEFORMAT" -d "$DURATION" -r "$SAMPLERATE" -c "$CHANNEL" "$ACCESSTYPEARG" "$OPMODEARG" -N --buffer-size=$BUFFERSIZE --period-size $PERIODSIZE
		;;
esac
